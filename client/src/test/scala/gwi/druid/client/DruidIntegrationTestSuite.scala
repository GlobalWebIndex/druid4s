...
class DruidIntegrationTestSuite extends AnyFreeSpec with ScalaFutures with Matchers with BeforeAndAfterAll with LazyLogging {
  ...
  private val brokerHost: String      = sys.env.getOrElse("BROKER_HOST", throw new IllegalStateException(s"BROKER_HOST env var must be defined !!!"))
  private val overlordHost: String    = sys.env.getOrElse("OVERLORD_HOST", throw new IllegalStateException(s"BROKER_HOST env var must be defined !!!"))
  ...
  def indexTestData(): Unit = {
    ...
    Await.ready(
      RanDaGen.run(
        ...
        Parallelism(4),
        JsonEventGenerator,
        EventConsumer("gcs", s"$sourceDataBucket@$targetDirPath", compress = true),

  ...
  "queries should serialize" in {
    List[Query](
      Query.timeSeries(
        ...
        s,
        ss,
        List(Aggregation.doubleMin(s, s)),
        Option(Filter.selector(s, s)),
        Option(PostAggregation.arithmetic(s, s, List(), Some(s))),
        Option.empty
      ),
      Query.groupBy(
        s,
        s,
        ss,
        List(Aggregation.cardinality(s, List(s), Some(true))),
        List(s),
        Option(GroupByLimitSpec.orderByAsc(s)),
        Option(HavingSpec.logicalAnd(List(s))),
        Option.empty,
        Option.empty,
        Option(Context(timeout = Option(1)))
      ),
      Query.topN(
        s,
        s,
        ...
        s,
        TopNMetric.inverted(s),
        10,
        Option.empty,
        Option(PostAggregation.fieldAccess(s, s)),
        Option.empty
      ),
      ...
      Query.segmentMetadata(
        s,
        List(s),
        Option(ToInclude.list(List(s)))
      ),
      Query.timeLowerBounded(
        s,
        Option(Context(useCache = Option(true)))
      ),
      Query.timeUpperBounded(
        s,
        Option(Context(queryId = Option(s)))
      ),
      Query.timeUnbounded(
        s,
        Option.empty

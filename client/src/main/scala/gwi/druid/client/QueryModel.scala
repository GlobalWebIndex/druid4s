...
case class ArithmeticPostAggregation(`type`: String, name: String, fn: String, fields: List[PostAggregation], ordering: Option[String] = Option.empty)
...
case class SegmentMetadataField(
    `type`: String,
    size: Long,
    cardinality: Option[Int],
    hasMultipleValues: Option[Boolean],
...
object Query {
  ...
  def groupBy(
    ...
    GroupByQuery("groupBy", dataSource, granularity, intervals, aggregations, dimensions: List[String], limitSpec, having, filter, postAggregation, context)
